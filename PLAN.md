...
- **换入**：ISessionProcessRunner 换桥实现；**M2 收尾（2026-08-02）legacy 反转：Rust 为默认后端，`dimi --legacy`（或 `DIMI_LEGACY=1`）退回 TS**；旧 `DIMI_RUST_EXEC/FS/ENV/WATCH/PTY` 开关已删除。
- **风险**：pty 语义（node-pty vs portable-pty）、Windows（parity 要求：TS 产品支持 Windows，Rust 版必须支持；实现顺序可后置，M6 前完成）。
- **切片进度**（每片 = Rust crate/模块 + napi bridge + TS 适配器 + 差分测试 + 双模式验证；切片 1-5 完成时用 `DIMI_RUST_*` 显式开关验证，M2 收尾反转默认）：
  ...
  - [x] 切片 2（2026-08-02 完成，commits 27025f097 + e1198160a）：`dimi-exec::fs`（readText/writeText/appendText/readBytes/writeBytes/readLines/createExclusive/stat/lstat/readdir/mkdir/remove/realpath，std-only 同步 + napi async 包装），bridge `RustFileSystem`/`RustReadLines`，`IHostFileSystem` 换入（`DIMI_RUST_FS=1`）。背压：exec 切片遗留的三层无界缓冲改为 sync_channel 容量 4（每管道 4×64KB，慢消费经内核管道缓冲背压子进程）。差分 16 场景 + 适配器 10 用例双模式全绿。Review 修复（e1198160a）：未知编码 → Err（先红后绿）、删死字段 pending_offset、readLines 改 async、binding-contract 补 RustFileSystem/RustReadLines 方法面。
  - [x] 切片 3（2026-08-02 完成，commit 09b932bb4）：`dimi-exec::env`（osKind/osArch 映射 aarch64→arm64、osVersion uname、shell 探测链、pathClass、homeDir），bridge `RustHostEnvironment`，`IHostEnvironment` 换入（`DIMI_RUST_ENV=1`）。差分 7 场景 + 换入 117/117 全绿。决策：login shell PATH 应用留在 TS（改 process.env 是 Node 副作用）；Windows shell/osVersion 占位，差分 win32 跳过 osVersion 断言。
  - [x] 切片 4（2026-08-02 完成，commit 78df3332f）：`dimi-exec::watch`（notify 7），bridge `RustFsWatch`/`RustFsWatchHandle`，`IHostFsWatchService` 换入（`DIMI_RUST_WATCH=1`）。语义对齐 chokidar：事件归一化 created/modified/deleted×file/directory、单路径 rename 探测、200ms 去重、`.git` 段过滤、词法/物理路径映射（FSEvents `/private/var` → 词法视图）。实测发现并修复 FSEvents 启动重放：watch 前快照（ignoreInitial 镜像）+ mtime 对比抑制重放的 Modify(Data)。差分 5 场景 + 适配器 6 用例（含 pre-existing 与 dispose 先红后绿）双模式全绿。
  - [x] 切片 5（2026-08-02 完成，commit 278b5e15a）：`dimi-exec::pty`（portable-pty 0.9），bridge `RustTerminal`/`RustTerminalProcess`（setOnData/setOnExit 双 tsfn + write/resize/kill），`IHostTerminalService` 换入（`DIMI_RUST_PTY=1`）。exit pump 50ms try_wait 轮询（kill 与 wait 共享 Child 句柄不死锁）；信号终止报 exitCode null（node-pty 报 128+信号，adapter 归一化 undefined→null）。差分 6 场景（echo/exit 42/cwd/resize/env/kill）+ 适配器 4 用例双模式全绿。遗留：Windows kill 走 portable-pty child.kill（未实测）；node-pty spawn-helper 在 pnpm store 丢失可执行位是环境问题（chmod +x 修复）。
  - **M2 遗留清单**：Windows 四项（进程 signal_to_number 编译、creation_flags 覆盖、shell /d /s、pty kill 实测）在 Windows 构建接入前处理（M6 前）；login shell PATH 应用留在 TS 适配器；agent-core-v2 `docs/state-manifest.d.ts` 测试失败为 HEAD 自带（rebrand 后未重新生成，与本迁移无关）。

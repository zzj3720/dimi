...
Dimi CLI 通过环境变量提供供应商 API 密钥、选择模型、控制运行时开关，以及迁移数据目录。供应商凭证不再写入 `config.toml`：可在 checkout 中用 `vp run dev:cli -- login <provider>` 安全保存密钥，也可为当前进程导出供应商的标准 API 密钥变量。
...
> 确保目录可写。多个 `dimi` 实例共用同一个 `DIMI_CODE_HOME` 会共享配置和凭证。
...
| `DIMI_CODE_BASE_URL`   | Dimi 托管服务与供应商感知 plugin 的 base URL | `https://api.kimi.com/coding/v1`   |
...
| `DIMI_CODE_PLUGIN_MARKETPLACE_URL`       | 覆盖 `/plugins` 加载的 plugin marketplace JSON，适合 dev loopback server、测试 CDN 文件或替换 marketplace 目录                                                                                                                                          | `https://github.com/zzj3720/dimi/releases/latest/download/plugins/marketplace.json`；也接受 `http://`、`file://` URL 和本地路径 |
...
| `DIMI_WEB_FETCH_BASE_URL`                | 网页抓取（`FetchURL`）服务的 API URL；优先级高于 `[services.moonshot_fetch] base_url`。文件中持久化的凭据和自定义 header 不会发送到环境变量指定的端点。环境变量和配置都没有指定端点时，已登录用户会先尝试 Dimi OAuth 托管抓取服务，再回退到本地直接请求 | 非空字符串；空白值被忽略                                                                               |
...
Dimi 会遵循标准代理环境变量，让所有出网流量——模型 API 调用、MCP 服务、网络工具、遥测、登录、更新检查——都走代理：
...
以 Node 子进程运行的 stdio MCP 服务，在其 Node 版本支持 `NODE_USE_ENV_PROXY` 时（Node ≥ 22.21 或 ≥ 24.5）会自动遵循 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`；SOCKS 代理仅作用于 Dimi 自身的流量。

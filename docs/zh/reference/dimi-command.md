...
`dimi` 是 Dimi CLI 的主命令，用于在终端中启动一次交互式会话。不带任何参数运行时，它会在当前工作目录下开启一个新会话；配合不同的 flag，可以续上历史会话、跳过审批、从 Plan 模式开始，或者指定自定义的 Skills 目录。
...
所有 flag 都是可选的，直接运行 `dimi` 即可进入交互式会话：
...
恢复会话时，可以通过 `--auto`、`--yolo` 或 `--plan` 覆盖原会话保存的权限或计划模式。例如，`dimi --continue --auto` 会恢复最近会话并切换到 auto 权限模式。
...
`--agent` 和 `--agent-file` 用于选择驱动新会话的 Agent，在 print 模式（`dimi -p`）和交互式 TUI 中均可使用：
...
在当前终端前台运行本地 Dimi 服务 —— 同一个进程同时挂载 REST + WebSocket API 与 web UI —— 并在服务就绪后用默认浏览器打开 web UI。命令会一直挂在终端，直到收到 `SIGINT` / `SIGTERM`（如 `Ctrl-C`）时干净退出。

服务运行时，`GET /openapi.json` 会返回 REST OpenAPI 文档，`GET /asyncapi.json` 会返回本地 WebSocket 协议的 AsyncAPI 文档。

```sh
dimi web                 # 前台运行服务并打开浏览器
dimi web --no-open       # 不打开浏览器
dimi web --port 58628    # 指定绑定端口
```
...
`dimi web` 默认只绑定本机 loopback 地址，并在启动横幅中打印 bearer token；web UI 通过 URL 的 `#token=` 片段自动完成鉴权。

::: info 提示
`dimi server` 命令树已废弃：任何 `dimi server …` 调用（含全部旧子命令）只会打印弃用提示并以退出码 1 结束，请改用 `dimi web`。唯一的例外是 `dimi server kill`，它仍然可用，仅用于停止 0.28.0 之前版本启动的服务。该提示将在 Dimi 下个大版本移除。
:::
...
已废弃——仅用于停止 0.28.0 之前的 Dimi 版本启动的服务。那些版本可能在后台遗留服务进程，记录在 legacy 单实例锁文件 `~/.dimi/server/lock` 中；该命令先请求 `POST /api/v1/shutdown` 优雅退出，再对锁中记录的 pid 发 SIGTERM、必要时升级为 SIGKILL，并在确认进程退出后删除锁文件。`dimi web` 启动的服务在前台运行，直接用 `Ctrl+C` 停止即可。
...
| 命令                        | 说明                                                         |
| --------------------------- | ------------------------------------------------------------ |
| `dimi doctor`               | 校验默认 `config.toml` 和 `tui.toml`                         |
| `dimi doctor config [path]` | 只校验 `config.toml`；传入 `path` 时使用该文件而不是默认文件 |
| `dimi doctor tui [path]`    | 只校验 `tui.toml`；传入 `path` 时使用该文件而不是默认文件    |
...
导出包含目标会话目录内的所有文件。全局诊断日志（`~/.dimi/logs/dimi.log`）默认包含，因为它可能含有其他会话或项目的事件；不想分享时加 `--no-include-global-log`。
...
本源码构建未配置发布通道。`dimi upgrade` 会提示自动升级不可用后退出；`dimi update` 是它的别名。

```sh
dimi upgrade
```

它绝不会回退到上游 Dimi 安装源。请用 `git pull --ff-only` 更新此 checkout，再运行 `vp install` 和 `vp run dev:cli`。

...
`dimi acp` switches Dimi CLI to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive dimi's sessions, prompts, and tool calls.
...
::: tip Who calls this?
You typically do not need to run `dimi acp` manually — this command is the subprocess entry point for IDEs. For IDE-side configuration, see [Using in IDEs](../guides/ides.md).
:::
...
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'Dimi CLI', version }`, capability matrix, and `authMethods` |
...
- `http` → dimi's `transport: 'http'` configuration
- `stdio` → dimi's `transport: 'stdio'` configuration
- `sse` → dimi's `transport: 'sse'` configuration
- `acp` → discarded with a warn log entry

## Next steps

- [Using in IDEs](../guides/ides.md) — Zed / JetBrains configuration steps and troubleshooting
- [`dimi` Command Reference](./dimi-command.md) — Complete subcommand list

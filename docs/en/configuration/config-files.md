...
Dimi CLI writes long-term runtime preferences — which connected provider/model to use, how much reasoning to request, and how many steps an agent can run per turn — into TOML (a plain-text configuration format with a clear structure) files. Provider credentials live in the provider runtime instead. Agent and runtime settings use `config.toml`; terminal-UI and client preferences (theme, editor, notifications, auto-update) use the companion `tui.toml`; user-owned provider definitions and overlays use `models.json`.

Default location: `~/.dimi/config.toml`, created automatically on first run.

## Config file location

The CLI reads configuration from `~/.dimi/config.toml`. To relocate the data directory, override it with the `DIMI_CODE_HOME` environment variable:

```sh
export DIMI_CODE_HOME=/path/to/dimi-home
```

The config file path then becomes `$DIMI_CODE_HOME/config.toml`. Regardless of where the directory lives, the file name is always `config.toml`.
...
Provider credentials are not config sections. Connect providers with `vp run dev:cli -- login` and inspect their models with `vp run dev:cli -- provider models`. Use `$DIMI_CODE_HOME/models.json` to add a custom provider or overlay a built-in model; it is a JSONC layer rather than a `config.toml` table. See [Providers and models](./providers.md#add-or-overlay-a-provider-with-modelsjson).

## `model_catalog`

`model_catalog` controls background refresh in the long-running server for authenticated providers with a remote model endpoint. The provider runtime also keeps a last-known-good cache and respects the upstream freshness response. It does not change locally declared `models.json` limits or capabilities.
...
This feature is experimental and disabled by default. Enable it with `DIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL=1`, or the master `DIMI_CODE_EXPERIMENTAL_FLAG=1`. It takes effect in every launch mode, including the interactive TUI.
...
`provider`, `model`, and `default_effort` can be overridden by `DIMI_SECONDARY_PROVIDER`, `DIMI_SECONDARY_MODEL`, and `DIMI_SECONDARY_EFFORT`, which take higher priority than `config.toml`.
...
| Field     | Type      | Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------- | --------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enabled` | `boolean` | `true`  | Whether Thinking is enabled by default for new sessions; set to `false` to request Thinking off when the selected model supports it. A model without `reasoning` ignores this preference, and an always-thinking model whose `thinkingLevelMap.off` is `null` keeps its declared default. |
| `effort`  | `string`  | —       | Preferred Thinking level (for example `low`, `medium`, `high`, `xhigh`, `max`). The selected model's `thinkingLevelMap` is authoritative: mapped levels are converted to the provider wire value; a mapped `null` is unavailable; a reasoning model with no map is on/off only. If the configured value is unavailable, the runtime uses the model default when declared, otherwise its normal supported mode. |
| `keep`    | `string`  | `"all"` | Preserved Thinking passthrough. On `dimi` it is sent as `thinking.keep`; on `anthropic` (Claude and Dimi's Anthropic-compatible mode) it is sent as a `context_management` `clear_thinking_20251015` edit (enabling keep routes Anthropic requests to the beta Messages API; an off-value disables keep and returns to the standard endpoint). `"all"` preserves prior turns' reasoning (`reasoning_content` / Anthropic thinking blocks); set to an off-value (`false`/`0`/`no`/`off`/`none`/`null`) to disable. Overridden by `DIMI_MODEL_THINKING_KEEP`; only injected while Thinking is on |
...
| `kill_grace_period_ms`            | `integer`                      | `5000`      | Grace period in milliseconds after session close, a manual stop, or a task timeout requests graceful termination. If a task is still running after this period, Dimi attempts to force-stop it                                                                                                                                                                                                                                                                                                                        |
...
`startup_timeout_ms` and `tool_timeout_ms` can be overridden by the `DIMI_MCP_STARTUP_TIMEOUT_MS` and `DIMI_MCP_TOOL_TIMEOUT_MS` environment variables respectively, which take higher priority than `config.toml`. See [MCP](../customization/mcp.md) for the full MCP server configuration.
...
`max_edge_px` can be overridden by the `DIMI_IMAGE_MAX_EDGE_PX` environment variable and `read_byte_budget` by `DIMI_IMAGE_READ_BYTE_BUDGET`; both take higher priority than `config.toml`.
...
::: tip
MCP server declarations are configured in `~/.dimi/mcp.json` or the project-local `.dimi/mcp.json`, not in `config.toml`. The interactive configuration entry point is `/mcp-config`; see [Model Context Protocol](../customization/mcp.md).
:::

## `tui.toml`

Alongside `config.toml`, the CLI keeps terminal-UI and client preferences in a companion `tui.toml` in the same directory (`~/.dimi/tui.toml`, or `$DIMI_CODE_HOME/tui.toml` when overridden). It is created with defaults on first run, and the interactive commands `/config`, `/theme`, and `/editor` write to it for you — so you rarely need to edit it by hand. If the file is malformed, the CLI falls back to defaults and shows a notice instead of failing to start.
...
| `[upgrade].auto_install`                 | `boolean`            | `true`      | Stored updater preference. It has no effect in this source build because no update channel is configured.                                                                                                                                                                                                             |
| `[status_line].items`                    | `string[]`           | `[]`        | Built-in slots to show on the first footer line and their order: `mode`, `model`, `tasks`, `cwd`, `git`, `tips`. Unset keeps the default layout; unknown ids are skipped with a warning                                                                                                                       |
...
[upgrade]
# Stored only in this source build; update the checkout with git pull instead.
auto_install = true
...
In addition to the user-level files under `~/.dimi`, Dimi reads a project-local configuration file at `<project-root>/.dimi/local.toml`. It holds settings that are specific to one project checkout and typically should not be shared with teammates.
...
Because directories are stored as absolute paths, which are specific to your machine, we recommend adding `.dimi/local.toml` to your project's `.gitignore` so it is not committed.
...
- [Environment variables](./env-vars.md) — complete list of runtime variables like `DIMI_CODE_HOME`

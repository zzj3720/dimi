...
Provider credentials are not config sections. Connect providers with `vp run dev:cli -- login` and inspect their models with `vp run dev:cli -- provider models`. Use `$KIMI_CODE_HOME/models.json` to add a custom provider or overlay a built-in model; it is a JSONC layer rather than a `config.toml` table. See [Providers and models](./providers.md#add-or-overlay-a-provider-with-modelsjson).

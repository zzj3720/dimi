...
Kimi Code CLI writes long-term runtime preferences — which connected provider/model to use, how much reasoning to request, and how many steps an agent can run per turn — into TOML (a plain-text configuration format with a clear structure) files. Provider credentials live in the provider runtime instead. Agent and runtime settings use `config.toml`; terminal-UI and client preferences (theme, editor, notifications, auto-update) use the companion `tui.toml`; user-owned provider definitions and overlays use `models.json`.
...
In addition to the user-level files under `~/.dimi`, Kimi Code reads a project-local configuration file at `<project-root>/.dimi/local.toml`. It holds settings that are specific to one project checkout and typically should not be shared with teammates.

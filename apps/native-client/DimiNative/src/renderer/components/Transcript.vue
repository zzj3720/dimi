  ...
  <main ref="scroller" class="transcript" tabindex="-1">
    ...
    <div v-if="state.entries.length === 0" class="welcome">
      ...
      <div class="suggestions">
        <div v-for="s in SUGGESTIONS" :key="s" class="suggestion-card" @click="suggest(s)">{{ s }}</div>
      </div>
      <div class="welcome-models">
        <div class="welcome-models-title">模型</div>
        <div class="model-row"><span class="m-name">5.3 Codex Spark</span><span class="m-level">轻度</span></div>
        <div class="model-row"><span class="m-name">5.6 Sol</span><span class="m-level">极高</span></div>
      </div>
    </div>

    <!-- Message thread -->
    <div v-else class="thread">
      <div v-for="e in state.entries" :key="e" class="entry" :class="'entry-' + e.kind">
        <!-- user -->
        ...
        <!-- tool -->
        <div v-else-if="e.kind === 'tool'" class="entry-tool clickable" @click="toggleTool(e.toolCallId)">
          <div class="body tool">
            <span class="tool-name">{{ e.text && e.text.length > 0 ? 'Used' : 'Using' }} {{ e.toolName }}</span>
          </div>
          <div v-if="e.args" class="body tool" style="font-family: ui-monospace, 'SF Mono', Menlo, monospace">{{ shellCmd(e.args) }}</div>
          <div
        ...
        <!-- status / compaction -->
        <div v-else class="body muted">{{ e.text }}</div>
      </div>
    </div>
  </main>
</template>

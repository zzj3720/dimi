...
describe('interactive provider login', () => {
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const home = await mkdtemp(join(tmpdir(), 'kimi-grok-dev-wrapper-e2e-'));
      tempHomes.push(home);
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const home = await mkdtemp(join(tmpdir(), 'kimi-update-channel-tui-e2e-'));
      tempHomes.push(home);
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const home = await mkdtemp(join(tmpdir(), 'kimi-malformed-models-tui-e2e-'));
      tempHomes.push(home);
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const home = await mkdtemp(join(tmpdir(), 'kimi-custom-provider-tui-e2e-'));
      tempHomes.push(home);
      await writeFile(join(home, 'tui.toml'), 'disable_paste_burst = true\n', 'utf8');

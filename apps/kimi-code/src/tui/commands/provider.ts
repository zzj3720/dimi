...
import { parseJsonc } from '@moonshot-ai/kimi-code-sdk';
...
async function promptCustomProvider(host: SlashCommandHost) {
  ...
  return new Promise<import('@moonshot-ai/kimi-code-sdk').CustomProviderInput | undefined>((resolve) => {
...
async function readCustomProviderDefinitions(
  path: string,
): Promise<readonly import('@moonshot-ai/kimi-code-sdk').CustomProviderInput[]> {
  ...
  if (providers !== undefined && typeof providers === 'object' && providers !== null && !Array.isArray(providers)) {
    return Object.entries(providers).map(([id, definition]) => ({
      ...(definition as import('@moonshot-ai/kimi-code-sdk').CustomProviderInput),
      id,
    }));
  }
  if (root?.['id'] !== undefined) {
    return [root as unknown as import('@moonshot-ai/kimi-code-sdk').CustomProviderInput];
  }

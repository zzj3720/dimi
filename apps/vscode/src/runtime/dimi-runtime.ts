...
export interface DimiRuntimeOptions {
...
export class DimiRuntime {
  readonly harness: DimiHarness;

  private readonly broadcast: RuntimeBroadcast;
  private readonly captureBaseline: DimiRuntimeOptions["captureBaseline"];
  private readonly log: DimiRuntimeOptions["log"];
  private readonly sessions = new Map<string, SessionRuntime>();
  private readonly sessionByView = new Map<string, string>();
  private closed = false;

  constructor(options: DimiRuntimeOptions) {
  ...
  private ensureOpen(): void {
    if (this.closed) throw new Error("Dimi runtime is closed.");
  }
}

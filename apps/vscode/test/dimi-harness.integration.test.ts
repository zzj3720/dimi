/**
 * Scenario: the VS Code host and another Node SDK client share one in-process Dimi home.
 ...
 * Wiring: DimiRuntime, DimiHarness, core, storage, and HTTP provider adapter are real; only the remote provider is local.
 * Run: pnpm --filter dimi exec vitest run test/dimi-harness.integration.test.ts
 */
...
import { DimiRuntime } from "../src/runtime/dimi-runtime";
...
async function createRuntimeRig(extraAliases: readonly string[] = []): Promise<RuntimeRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "dimi-vscode-harness-"));
  ...
  const harness = createDimiHarness({
    homeDir,
    identity: { userAgentProduct: "dimi-vscode", version },
    providerRuntime: createTestProviderRuntime({
      ...
      model: {
        ...
        headers: { "User-Agent": `dimi-vscode/${version}` },
      },
    }),
  });
  const broadcasts: BroadcastRecord[] = [];
  const logs: LogRecord[] = [];
  const runtime = new DimiRuntime({
    version,
    homeDir,
    harness,
...
async function createPlainHarness(
  ...
  const harness = createDimiHarness({
    homeDir,
    identity: { userAgentProduct: "dimi-cli", version: "test" },
    providerRuntime:
      provider === undefined
        ? undefined
...
async function createMcpHandlerRig(): Promise<McpHandlerRig> {
  const homeDir = await mkdtemp(join(tmpdir(), "dimi-vscode-mcp-handler-"));
...
describe("VS Code Dimi harness integration (shares one in-process SDK home)", () => {
  ...
  it("sends the package version in User-Agent when VS Code prompts the provider", async () => {
    ...
    expect(rig.provider.requests[0]?.headers["user-agent"]).toBe(`dimi-vscode/${rig.version}`);
  });
  ...
  it("exports current context as Markdown under the workspace", async () => {
    ...
    expect(markdown).toContain("# Dimi Session Export");

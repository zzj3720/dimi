/**
 * Scenario: the VS Code host owns one Dimi harness and routes Webviews to shared SDK sessions.
 * Responsibilities: create/resume/switch, per-session settings, multi-view ownership, detach, and disposal.
 * Wiring: DimiRuntime and SessionRuntime are real; in-memory Session/DimiHarness fakes form the public SDK boundary.
 * Run: pnpm exec vitest run --config apps/vscode/vitest.config.ts test/dimi-runtime.test.ts
 */
...
import { DimiRuntime, type OpenSessionOptions } from "../src/runtime/dimi-runtime";
...
describe("Dimi runtime (owns shared SDK sessions for Webviews)", () => {
  ...
  it("announces the session's actual status to the attaching view so the display matches it", async () => {
    ...
    expect(broadcasts).toContainEqual({
      ...
      data: {
        type: "StatusUpdate",
        payload: { model: "dimi-test", thinking_effort: "max", plan_mode: true },
        _sessionId: "saved-1",
      },
      webviewId: "view-1",
    });

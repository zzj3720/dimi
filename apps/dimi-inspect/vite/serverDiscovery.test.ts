...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'dimi-inspect-discovery-'));
});
...
describe('resolveDimiHomeDir', () => {
  it('honors DIMI_CODE_HOME, else falls back to ~/.dimi', () => {
    expect(resolveDimiHomeDir({ DIMI_CODE_HOME: '/tmp/kh' })).toBe('/tmp/kh');
    expect(resolveDimiHomeDir({})).toBe(join(process.env['HOME'] ?? '', '.dimi'));
  });
});

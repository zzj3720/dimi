...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'dimi');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'dimi')
      : pathWin32.join(home, 'AppData', 'Local', 'dimi', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'dimi');
}
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.dimi-native-entry.cjs');
  ensureFile(
    entryPath,

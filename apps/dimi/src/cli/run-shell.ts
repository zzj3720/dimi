...
export async function runShell(opts: CLIOptions, version: string): Promise<void> {
  ...
  const harnessOptions: DimiHarnessOptions = {
  ...
  const harness = createDimiHarness(harnessOptions);
  log.info("dimi starting", {
    version,
    uiMode: CLI_UI_MODE,
  ...
  const trackLifecycleForSession = (
    sessionId: string,
    event: string,
    properties?: Parameters<DimiHarness["track"]>[1],
  ) => {
  ...
  const trackLifecycle = (event: string, properties?: Parameters<DimiHarness["track"]>[1]) => {
  ...
  // If we crash without going through DimiTUI.stop(), the terminal is left in
  ...
  tui.onExit = async (exitCode = 0) => {
    ...
    if (sessionId !== "" && hasContent) {
      hints.push(`${gutter}To resume this session: dimi -r ${sessionId}`);
    }

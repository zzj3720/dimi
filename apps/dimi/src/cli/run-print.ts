/**
 * Native `dimi -p` (print mode) runner.
 *
...
import { createDimiDefaultHeaders, createDimiDeviceId } from "@dimi-agent/dimi-oauth";
...
export function requireConfiguredModel(...models: readonly (string | undefined)[]): string {
  ...
  if (model === undefined) {
    throw new Error(
      "No model configured. Run `dimi` and use /login to sign in, then retry; or set default_model in config.toml.",
    );
  }
  return model;
}
...
export async function runPrint(
  ...
  createDimiDeviceId(homeDir, {
    onFirstLaunch: () => {
      firstLaunch = true;
    },
  });
  const logging = resolveLoggingConfig({ homeDir, env: process.env });
  const identity = createDimiCodeHostIdentity(version);
  const hostHeaders = createDimiDefaultHeaders({ homeDir, ...identity });
  ...
  try {
    // Dimi does not collect telemetry: `ITelemetryService` stays on its
    // built-in null appender, so every track call is a no-op. The service is
    // still resolved so `setContext` / `track2` call sites keep their shape.
    ...
    if (target.cwd !== undefined && resolve(target.cwd) !== resolve(workDir)) {
      stderr.write(
        `Session "${opts.session}" was created under a different directory.\n` +
          `  cd "${target.cwd}" && dimi -r ${opts.session}\n\n`,
      );

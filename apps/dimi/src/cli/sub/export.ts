...
export function registerExportCommand(parent: Command, deps?: Partial<ExportDeps>): void {
  parent
    ...
    .option(
      ...
      'Skip bundling the active global diagnostic log (~/.dimi/logs/dimi.log, not rotated .1 files). By default the global log is included.',
    )
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps {
  let harness: DimiHarness | undefined;
  ...
  const getHarness = (): DimiHarness => {
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    harness ??= createDimiHarness({

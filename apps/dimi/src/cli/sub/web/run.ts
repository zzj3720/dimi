/**
 * `dimi web` — run the local server in the foreground and open the web UI.
 *
...
import { hostRequestHeadersSeed } from '@dimi-agent/agent-core-v2';
import { createServerLogger, startServer, type ServerLogger } from '@dimi-agent/kap-server';
import { shutdownTelemetry, track } from '@dimi-agent/dimi-telemetry';
...
function formatReadyLine(
  ...
  const notice = dangerousBypassAuth
    ...
    : '';
  return `${notice}Dimi server: ${buildOpenableUrl(origin, token)}\n`;
}
...
/**
 * `dimi web` — runs the local server in-process, attached to the current
...
async function runServerInProcess(
  ...
  const v2 = await startServer({
    ...
    telemetry: true,
    // Seed the CLI's Dimi identity headers so the engine's outbound
    // requests (model, WebSearch, FetchURL) carry the same User-Agent +
    // X-Msh-* identity as direct CLI runs.
    seeds: hostRequestHeadersSeed(buildDimiDefaultHeaders(version)),
    webAssetsDir,
  });
 ...
 * and the web UI is expected to come from the dimi-web Vite dev server.
...
export function formatReadyBanner(
  ...
  // Borderless header: the Dimi sprite (the little mascot with eyes) sits next
  // to the title, keeping the brand without the enclosing box.
  const logo = ['▐█▛█▛█▌', '▐█████▌'] as const;
  const lines: string[] = [
    '',
    `  ${primary(logo[0])}  ${title('Dimi server ready')}  ${dim(getVersion())}`,

 ...
 *
 * Wire-up:
 *  - A {@link DimiHarness} is constructed with the dimi host identity
...
} from '@dimi-agent/acp-adapter';
import { createDimiHarness, type Session, type SkillSummary } from '@dimi-agent/dimi-sdk';
...
export function registerAcpCommand(parent: Command): void {
  parent
    .command('acp')
    .description('Run dimi as an Agent Client Protocol (ACP) server over stdio.')
    .option(
      '--login',
    ...
    .action(async (opts: { login?: boolean }) => {
      ...
      const identity = createDimiCodeHostIdentity();
      const harness = createDimiHarness({
        identity,
        uiMode: 'acp',
      });
      // Forward `DIMI_CODE_HOME` (if set) into `authMethods[0].env` so the
      // `dimi login` subprocess clients spawn for terminal-auth writes its
      ...
      // `dimi login` subcommand — matches dimi-cli `acp/server.py:77-96`.
      ...
      try {
        await runAcpServer(harness, {
          agentInfo: { name: 'Dimi CLI', version: getVersion() },

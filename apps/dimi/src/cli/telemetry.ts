...
import { createDimiDeviceId } from "@dimi-agent/dimi-oauth";
import { ConfigRegistry } from "@dimi-agent/agent-core-v2";
...
export interface InitializeCliTelemetryOptions {
  readonly harness: Pick<DimiHarness, "homeDir" | "auth" | "track">;
  readonly bootstrap: CliTelemetryBootstrap;
  readonly config: Pick<DimiConfig, "defaultModel" | "telemetry">;
...
export function createCliTelemetryBootstrap(): CliTelemetryBootstrap {
  ...
  const deviceId = createDimiDeviceId(homeDir, {
    onFirstLaunch: () => {
      firstLaunch = true;
    },
  });
...
/**
 * Bootstrap telemetry for the `dimi web` host.
 *
 ...
 * the same real client into `DimiCore`, so agent-core events emitted inside the
 ...
 *
 * The returned client wraps the `@dimi-agent/dimi-telemetry` module
...
function readServerTelemetryConfig(
  configPath: string,
): Pick<DimiConfig, "telemetry" | "defaultModel"> {
  try {

...
import { effectiveModelAlias } from '@dimi-agent/dimi-sdk';
...
export class WelcomeComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (safeWidth < 24) {
      const title = chalk.bold.hex(currentTheme.palette.primary)('Welcome to Dimi!');
    ...
    const rightRow0 = truncateToWidth(
      chalk.bold.hex(currentTheme.palette.primary)('Welcome to Dimi!'),
      textWidth,
      '…',
    );

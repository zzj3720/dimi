...
const mocks = vi.hoisted(() => ({
  ...
  resolveDimiHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/dimi-export-home'),
...
vi.mock('@dimi-agent/dimi-sdk', async (importOriginal) => {
  ...
  return {
    ...
    createDimiHarness: (...args: unknown[]) => {
      ...
      const homeDir = options?.homeDir ?? '/tmp/dimi-export-home';
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'dimi-export-'));
});
...
afterEach(() => {
  ...
  mocks.resolveDimiHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/dimi-export-home',
  );
...
describe('dimi export', () => {
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'dimi', 'export', '--no-include-global-log', '-y']);
  ...
  it('initializes and flushes telemetry around default export tracking', async () => {
    ...
    await program.parseAsync(['node', 'dimi', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'dimi', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'dimi', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
    ...
    expect(mocks.dimiHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/dimi-export-home' }),
    );

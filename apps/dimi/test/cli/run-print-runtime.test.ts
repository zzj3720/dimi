...
const mocks = vi.hoisted(() => ({
  ...
  createDimiDefaultHeaders: vi.fn(() => ({})),
  resolveDimiHome: vi.fn((homeDir?: string) => homeDir ?? "/tmp/dimi-test-home"),
  createDimiDeviceId: vi.fn(() => "device-1"),
}));

vi.mock("@dimi-agent/agent-core-v2", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@dimi-agent/agent-core-v2")>();
  return {
    ...actual,
...
vi.mock("@dimi-agent/dimi-oauth", async () => {
  ...
  return {
    ...actual,
    createDimiDefaultHeaders: mocks.createDimiDefaultHeaders,
...
describe("runPrint", () => {
  ...
  it("submits a prompt, renders native events, awaits completion, and drains", async () => {
    ...
    expect(stderr.write).toHaveBeenNthCalledWith(1, "dimi version 1.2.3-test\n");
  ...
  it("binds the profile named by --agent-file when --agent is absent", async () => {
    const dir = await mkdtemp(join(tmpdir(), "dimi-agent-file-"));

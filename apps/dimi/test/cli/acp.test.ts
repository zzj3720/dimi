 ...
 * that the action wires the harness into `@dimi-agent/acp-adapter`'s
...
vi.mock("@dimi-agent/acp-adapter", () => ({
...
describe("dimi acp", () => {
  ...
  it("invokes runAcpServer with a constructed harness and exits 0 on success", async () => {
    ...
    await expect(program.parseAsync(["node", "dimi", "acp"])).rejects.toThrow(ExitCalled);
    ...
    expect(optsArg).toEqual(
      expect.objectContaining({
        agentInfo: { name: "Dimi CLI", version: expect.any(String) },
      }),
    );
    expect(exitSpy).toHaveBeenCalledWith(0);
  });

  it("forwards DIMI_CODE_HOME to terminalAuthEnv when set", async () => {
    const previous = process.env["DIMI_CODE_HOME"];
    process.env["DIMI_CODE_HOME"] = "/tmp/dimi-debug";
    try {
      const program = new Command("dimi").exitOverride();
      registerAcpCommand(program);
      ...
      expect(optsArg).toEqual(
        expect.objectContaining({
          terminalAuthEnv: { DIMI_CODE_HOME: "/tmp/dimi-debug" },
        }),
      );
    } finally {
      if (previous === undefined) {
        ...
        process.env["DIMI_CODE_HOME"] = previous;
      }
    }
  });

  it("omits terminalAuthEnv when DIMI_CODE_HOME is unset", async () => {
    const previous = process.env["DIMI_CODE_HOME"];
    delete process.env["DIMI_CODE_HOME"];
    try {
      const program = new Command("dimi").exitOverride();
  ...
  it("exits without starting the ACP server when --login is passed", async () => {
    ...
    const loginStub = vi.fn(async () => ({ providerName: "dimi" }));
    vi.doMock(import("@dimi-agent/dimi-sdk"), async (importOriginal) => {
      ...
      return {
        ...
        createDimiHarness: () =>
          ...
          }) as unknown as ReturnType<typeof actual.createDimiHarness>,
      };
    });
    vi.resetModules();
    ...
    try {
      ...
      await expect(program.parseAsync(["node", "dimi", "acp", "--login"])).rejects.toThrow(
        ExitCalled,
      );
      ...
      vi.doUnmock("@dimi-agent/dimi-sdk");
      vi.resetModules();
    }
  });
});

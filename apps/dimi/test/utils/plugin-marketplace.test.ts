...
describe('loadPluginMarketplace', () => {
  it('loads a local marketplace file and resolves relative plugin sources', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dimi-plugin-marketplace-'));
      ...
      plugins: [
        {
          ...
          version: '1.0.0',
          description: 'Datasource tools',
          source: join(dir, 'dimi-datasource'),
          keywords: ['data'],
          homepage: undefined,
        },
        {
  ...
  it('includes Superpowers in the repository marketplace fixture', async () => {
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'dimi-datasource',
        tier: 'official',
        source: join(REPO_ROOT, 'plugins/official/dimi-datasource'),
      }),
    );
  });
          ...
          plugins: [
            {
              id: 'dimi-datasource',
              displayName: 'Dimi Datasource',
              source: './official/dimi-datasource.zip',
            },
          ],
        }),
    })) as unknown as typeof fetch;
  ...
  describe('version derivation from a GitHub source', () => {
    ...
    it('does not derive a version from a non-GitHub URL', async () => {
      const entry = await loadEntry('https://github.com/zzj3720/dimi/releases/latest/download/curated-superpowers.zip');
      expect(entry.version).toBeUndefined();
    });
  ...
  it('accepts legacy marketplace type aliases as normal plugins', async () => {
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'dimi-webbridge',
        source: join(dir, 'dimi-webbridge'),
      }),
    );
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({

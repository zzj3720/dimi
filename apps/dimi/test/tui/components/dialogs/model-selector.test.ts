...
describe("ModelSelectorComponent", () => {
  ...
  it("lays out the provider as a right column and marks the current model", () => {
    ...
    expect(out).toMatch(/❯ Kimi K2\s+Dimi ← current/);
    // Provider is no longer inlined in parentheses next to the name.
    expect(out).not.toContain("Kimi K2 (Dimi)");
  });

  it('toggles thinking with Left/Right (not with "/")', () => {
    const onSelect = vi.fn();
    const picker = new ModelSelectorComponent({
      models: { dimi: model("Kimi K2", ["thinking"]) },
      currentValue: "dimi",
      currentThinkingEffort: "on",
      onSelect,
      onCancel: vi.fn(),
    ...
    expect(onSelect).toHaveBeenLastCalledWith({ alias: "dimi", thinking: "on" });

    // Right arrow flips the draft (true -> false).
    picker.handleInput(RIGHT);
    picker.handleInput("\r");
    expect(onSelect).toHaveBeenLastCalledWith({ alias: "dimi", thinking: "off" });
  ...
  it("forces always-thinking models on and unsupported models off", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        always: model("Dimi Thinking", ["always_thinking"]),
        plain: model("Dimi Plain", ["tool_use"]),
      },
      currentValue: "always",
      currentThinkingEffort: "off",
      onSelect,
  ...
  it("renders the unavailable thinking segment muted", () => {
    const picker = new ModelSelectorComponent({
      models: { always: model("Dimi Thinking", ["always_thinking"]) },
      currentValue: "always",
      currentThinkingEffort: "on",
      onSelect: vi.fn(),
      onCancel: vi.fn(),
  ...
  it("keeps the thinking draft when moving across models", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        plain: model("Dimi Plain", ["tool_use"]),
        thinking: model("Dimi Thinking", ["thinking"]),
      },
      currentValue: "plain",
      currentThinkingEffort: "off",
      onSelect,
  ...
  it("defaults a thinking-capable model to On but keeps the current model state", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        current: model("Dimi Current", ["thinking"]),
        other: model("Dimi Other", ["thinking"]),
      },
  ...
  it("fuzzy-filters by typing and reports a match count", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: { k2: model("Kimi K2"), turbo: model("Dimi Turbo") },
      currentValue: "k2",
      currentThinkingEffort: "off",
      searchable: true,
      onSelect: vi.fn(),
    ...
    expect(out).toContain("Dimi Turbo");
  ...
  it("invokes onSessionOnlySelect on Alt+S with the effective thinking state", () => {
    ...
    expect(onSessionOnlySelect).toHaveBeenCalledWith({ alias: "dimi", thinking: "off" });
  ...
  it("renders effort segments with the default effort highlighted", () => {
    const picker = new ModelSelectorComponent({
      models: { dimi: effortModel("Kimi K2", ["low", "high", "max"], "high") },
      currentValue: "dimi",
  ...
  it("cycles efforts with Left/Right and clamps at the ends", () => {
    ...
    picker.handleInput(RIGHT);
    picker.handleInput("\r");
    expect(onSelect).toHaveBeenLastCalledWith({ alias: "dimi", thinking: "max" });
  ...
  it("always-on effort models hide Off and clamp selection at the last effort", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        dimi: effortModel("Kimi K2", ["low", "high", "max"], "high", ["always_thinking"]),
      },
      currentValue: "dimi",
      currentThinkingEffort: "high",
      onSelect,
  ...
  it("defaults an effort model without a current level to its defaultEffort", () => {
    ...
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel("Dimi Other", ["low", "high", "max"], "max"),
      },
      currentValue: "current",
      currentThinkingEffort: "off",
      onSelect,
  ...
  it("falls back to the middle effort when an effort model has no defaultEffort", () => {
    const picker = new ModelSelectorComponent({
      models: {
        other: effortModel("Dimi Other", ["low", "medium", "high"]),
      },
      currentValue: "current",
      currentThinkingEffort: "off",
      onSelect: vi.fn(),

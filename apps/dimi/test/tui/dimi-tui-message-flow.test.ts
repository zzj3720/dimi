...
import { deleteAllKittyImages, resetCapabilitiesCache, setCapabilities } from '@dimi-agent/pi-tui';
import type {
  ApprovalRequest,
  ApprovalResponse,
  Event,
...
async function makeDriver(
  ...
  const driver = new DimiTUI(harness as never, makeStartupInput()) as unknown as MessageDriver;
...
async function makeTempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'dimi-tui-'));
  tempDirs.push(dir);
  return dir;
}

async function makeExportedSessionZip(content = 'session zip'): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'dimi-feedback-export-'));
  tempDirs.push(dir);
...
`,
  ...
  it('shows an error instead of throwing when skill media materialization fails', async () => {
    ...
    const missing = imageStore.addVideo('video/quicktime', '/tmp/dimi-missing-source.mov');
  ...
  it('shows an error instead of throwing when plugin command media materialization fails', async () => {
    ...
    const { driver } = await makeDriver(session);
    const imageStore = (driver as unknown as { imageStore: ImageAttachmentStore }).imageStore;
    const missing = imageStore.addVideo('video/mp4', '/tmp/dimi-missing-source.mp4');
  ...
  it('appends the /export-debug-zip hint beneath session error messages', async () => {
    ...
    expect(transcript).not.toContain('dimi export');
  });
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ Dimi');
  ...
  it('toggles plugin MCP servers from the text command', async () => {
    ...
    driver.handleUserInput('/plugins mcp enable dimi-datasource data');
  ...
  it('installs from a positional source on /plugins install after trusting it', async () => {
    ...
    driver.handleUserInput('/plugins install ./plugins/dimi-datasource');
    ...
    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        resolve('/tmp/proj-a', './plugins/dimi-datasource'),
      );
    });
  });
  ...
  it('shows a quota note after installing a quota-consuming official plugin', async () => {
    ...
    driver.handleUserInput(
      '/plugins install https://github.com/zzj3720/dimi/releases/latest/download/dimi-datasource.zip',
    );
  ...
  it('does not show the quota note for a same-id fork installed from a local path', async () => {
    ...
    driver.handleUserInput('/plugins install ./plugins/dimi-datasource-fork');
    ...
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
      expect(transcript).toContain('Installed Dimi Datasource');
    });
    expect(stripSgr(renderTranscript(driver))).not.toContain(
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
        ...
        plugins: [
          {
            ...
            source: 'https://github.com/zzj3720/dimi/releases/latest/download/dimi-datasource.zip',
          },
        ],
      }),
      'utf8',
    ...
    await vi.waitFor(() => {
      expect(stripSgr(panel.render(120).join('\n'))).toContain('Dimi Datasource');
    });
    // The pinned Dimi WebBridge row leads the Official tab, so move down to
    // the Dimi Datasource entry before installing.
    panel.handleInput('\u001B[B');
    panel.handleInput('\r');

    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        'https://github.com/zzj3720/dimi/releases/latest/download/dimi-datasource.zip',
      );
    });
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
  ...
  it('returns to the plugin list when a marketplace install fails', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(rendered).toContain('Dimi Datasource');
  ...
  it('removes a plugin record without auto-running any cleanup skill', async () => {
    ...
    driver.handleUserInput('/plugins remove dimi-webbridge');
    ...
    await vi.waitFor(() => {
      expect(session.removePlugin).toHaveBeenCalledWith('dimi-webbridge');
    });
    expect(session.activateSkill).not.toHaveBeenCalled();
  });
  ...
  it('toggles plugin MCP servers from the overview MCP picker', async () => {
    ...
    const session = makeSession({
      ...
      getPluginInfo: vi.fn(async () => ({
        ...
        mcpServers: [
          {
            name: 'metadata',
            runtimeName: 'plugin-dimi-datasource-metadata',
            enabled: serverEnabled.get('metadata') === true,
            transport: 'stdio',
            command: 'node',
            args: ['./bin/dimi-datasource.mjs', 'metadata'],
          },
          {
            name: 'data',
            ...
            transport: 'stdio',
            command: 'node',
            args: ['./bin/dimi-datasource.mjs', 'data'],
          },
        ],
        diagnostics: [],
      })),
    ...
    expect(stripSgr(renderTranscript(driver))).not.toContain(
      'Disabled MCP server data for dimi-datasource. Run /reload or /new to apply.',
    );
  });
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/Kimi K2\s+Dimi ← current/);
    expect(pickerOutput).toMatch(/❯ Dimi Turbo\s+Dimi/);
    ...
    expect(filteredOutput).toContain('Dimi Turbo');
  ...
  it('uses the effective effort returned after a model-switch fallback', async () => {
    ...
    expect(driver.state.appState.model).toBe('kimi-coding/turbo');
    expect(driver.state.appState.thinkingEffort).toBe('mid');
    expect(renderTranscript(driver)).toContain('Switched to dimi-turbo with thinking mid.');
  });
  ...
  it('refreshes all configured provider models before opening /model picker', async () => {
    ...
    const tui = driver as unknown as DimiTUI;
  ...
  it('enables search in the shared model selector helper', async () => {
    ...
    expect(output).toContain('Dimi Turbo');
    expect(output).not.toContain('Dimi Alpha');
  ...
  it('updates terminal title through pi-tui without changing process title', async () => {
    ...
    try {
      ...
      expect(process.title).toBe('dimi-test-runner');
    } finally {
  ...
  it('forks the active session and switches to the returned session', async () => {
    ...
    try {
      ...
      expect(driver.state.transcriptContainer.render(120).join('\n')).toContain(
        'Session forked (ses-fork). To return to the original session: dimi -r ses-source',
      );
    } finally {
      process.title = originalTitle;
    }
...
describe('/effort support_efforts override', () => {
  it('warns and applies efforts hidden by an Anthropic support_efforts override', async () => {
    ...
    const { driver } = await makeDriver(session, {
      getConfig: vi.fn(async () => ({
        providers: {
          compatible: { type: 'dimi', apiKey: 'test-key' },
        },
        models: {
          k2: {
            provider: 'compatible',
  ...
  it('offers no fallback efforts for an unknown model on a Dimi provider using the Anthropic protocol', async () => {
    const { driver } = await makeDriver(makeSession(), {
      getConfig: vi.fn(async () => ({
        providers: {
          compatible: { type: 'dimi', apiKey: 'test-key' },
  ...
  it('keeps rejecting efforts hidden by a Dimi support_efforts override', async () => {
    const session = makeSession();
    const { driver } = await makeDriver(session, {
      getConfig: vi.fn(async () => ({
        providers: {
          dimi: { type: 'dimi', apiKey: 'test-key' },
        },
        models: {
          k2: {
            provider: 'dimi',
    ...
    driver.state.appState.model = 'dimi/k2';

    driver.handleUserInput('/effort max');

    await vi.waitFor(() => {
      expect(renderTranscript(driver)).toContain(
        'Unsupported thinking effort "max" for dimi/k2. Available: off, low, high',
      );
    });
    expect(session.setThinking).not.toHaveBeenCalled();
  });

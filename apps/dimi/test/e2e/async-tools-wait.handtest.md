...
Run this flow with the `background-bash-stdin` experiment enabled. A deterministic local model endpoint is suitable as long as the request still enters through `dimi -p` or the interactive TUI and emits real `Bash` and `TaskInput` tool calls.

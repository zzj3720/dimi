 ...
 * `DimiHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 ...
 *   DIMI_E2E_WORKDIR  — workspace directory (default: /tmp/dimi-e2e)
 */

import { mkdirSync } from "node:fs";
import process from "node:process";

import { createDimiHarness, type Event } from "@dimi-agent/dimi-sdk";
import { describe, expect, test } from "vitest";

import { createDimiCodeHostIdentity, getVersion } from "#/cli/version";

const DEFAULT_PROMPT = "Reply with a single word: hi";
const DEFAULT_WORKDIR = "/tmp/dimi-e2e";
...
describe.skipIf(!ENABLED)("SDK e2e — real LLM smoke", () => {
  test(
    "round-trips a single prompt through DimiHarness",
    async () => {

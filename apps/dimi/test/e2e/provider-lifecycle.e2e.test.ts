...
describe("provider CLI lifecycle", () => {
  it("keeps the production built-in catalog usable and reports malformed models.json", async () => {
    const home = await mkdtemp(join(tmpdir(), "dimi-malformed-models-cli-e2e-"));
    tempHomes.push(home);
  ...
  it("runs Grok 4.5 through the production CLI with its declared context and thinking wire shape after restart", async () => {
    const home = await mkdtemp(join(tmpdir(), "dimi-grok-cli-e2e-"));
    tempHomes.push(home);
    const seen: unknown[] = [];
    ...
    try {
      ...
      expect(JSON.stringify(seen[0])).not.toContain("Dimi CLI");
      expect(JSON.stringify(seen[0])).not.toContain("You are Dimi");
  ...
  it("persists a localhost dynamic catalog through provider refresh and a fresh CLI process", async () => {
    const home = await mkdtemp(join(tmpdir(), "dimi-dynamic-catalog-cli-e2e-"));
    tempHomes.push(home);
    let modelRequests = 0;
    const server = createServer((request, response) => {
  ...
  it("removes the final custom model from a built-in overlay without removing its built-in catalog", async () => {
    const home = await mkdtemp(join(tmpdir(), "dimi-overlay-model-remove-cli-e2e-"));
    tempHomes.push(home);
    await writeFile(join(home, "models.json"), JSON.stringify({
  ...
  it("lists, logs in, reads built-ins, logs out, and exits cleanly through the production entry point", async () => {
    const home = await mkdtemp(join(tmpdir(), "dimi-provider-e2e-"));
    tempHomes.push(home);
    ...
    await expect(readJson(join(home, "models.json"))).resolves.toMatchObject({
      providers: { local: { headers: { "x-client": "dimi" }, models: [expect.objectContaining({ contextWindow: 32_000 })] } },
    });
...
function runCliPty(
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn("python3", ["-u", "-c", PTY_BRIDGE, ...command], {
      cwd: appRoot,
      env: { ...env, DIMI_CODE_HOME: home, DIMI_LOG_LEVEL: "off", TERM: "xterm-256color" },
      stdio: ["pipe", "pipe", "pipe"],
    });
    let output = "";
    let sent = false;
...
function sanitizedProviderEnv(overrides: Readonly<Record<string, string>> = {}): NodeJS.ProcessEnv {
  ...
  for (const name of [
    ...
    "GROQ_API_KEY", "HF_TOKEN", "DIMI_API_KEY", "MINIMAX_API_KEY", "MINIMAX_CN_API_KEY",

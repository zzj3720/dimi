...
Dimi is an AI coding agent that runs in your terminal. It can read and edit code, run shell commands, search files, fetch web pages, and choose the next step based on the feedback it receives. It works out of the box with Moonshot AI's Dimi models and can connect to other supported providers.
...
For updates, use `git pull --ff-only && vp install`; `dimi upgrade` checks the Dimi GitHub Releases for newer versions.

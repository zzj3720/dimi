...
export default defineConfig({
  plugins: [
    ...
    Icons({
      compiler: 'vue3',
      // Local Dimi Design System icons (24×24 outlined, fill="currentColor"),
      // copied from the design-system icon pack into src/icons/dimi/ and
      // imported as `~icons/dimi/<file-name>` (plus `?raw`), same as the ri
      // collection. Registered in src/lib/icons.ts only.
      customCollections: {
        dimi: FileSystemIconLoader(fileURLToPath(new URL('./src/icons/dimi', import.meta.url))),
      },
    }),
  ],
  ...
  define: {
    ...
    // True only for the web bundle embedded in the Dimi Desktop app (set by the

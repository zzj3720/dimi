...
A browser client for Dimi — a peer to the TUI (`apps/dimi`) that talks
...
WEB_PORT=5197 DIMI_SERVER_URL=http://192.168.97.91:58627 pnpm -C apps/dimi-web run dev
#   …or from the repo root:  pnpm dev:web   (uses the defaults below)

# checks
pnpm -C apps/dimi-web run typecheck     # vue-tsc --noEmit
pnpm -C apps/dimi-web run test          # vitest (pure logic only)
pnpm -C apps/dimi-web run build         # vite build
```
...
server (REST + WS)
  ...
  └─ src/composables/useDimiWebClient.ts   the ONLY place that imports api + state;
...
- **i18n** (`src/i18n/`): vue-i18n, en/zh, per-namespace flat camelCase keys.
  Detect order: `localStorage('dimi-locale')` → `navigator.language` → `en`.
---
...
Dimi Web is **not published as a standalone package**. It ships as the built-in
web UI of the `dimi` CLI (`apps/dimi`).

### Current release flow

1. **Develop** — `pnpm dev:web` (or `pnpm -C apps/dimi-web run dev`).
2. **Build** — `pnpm -C apps/dimi-web run build` produces `apps/dimi-web/dist`.
3. **Bundle into CLI** — `pnpm -C apps/dimi run build` runs
   `scripts/copy-web-assets.mjs`, which copies `apps/dimi-web/dist` into
   `apps/dimi/dist-web`.
4. **Publish** — the root `.github/workflows/release.yml` publishes
   `@dimi-agent/cli` to npm; `dist-web` is listed in the package `files`
   array, so the built web assets travel with the CLI package.
5. **Serve** — `dimi web` serves `dist-web` from the
   installed package.

The web UI does not display its own package version or build commit. It is
bundled into the CLI package and follows the published `@dimi-agent/cli`
release.

### Suggested improvements

- **Keep the current coupling for now.** Because Dimi is primarily a local
...
- **Add an independent web-deploy workflow only when needed.** If a public
  ...
  `.github/workflows/web-deploy.yml` that builds `apps/dimi-web` and uploads
...
- **Keep versioning owned by the CLI release.** `apps/dimi-web/package.json`
...
- **Ensure the web build is exercised in CI.** The root `build` script already
  builds every workspace, so `pnpm run build` in CI covers `apps/dimi-web`.

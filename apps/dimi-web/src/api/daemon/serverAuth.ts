// apps/dimi-web/src/api/daemon/serverAuth.ts
...
//   1. From the URL fragment (`#token=<...>`) that `dimi web` appends when it
...
// `dimi web rotate-token` invalidates a stale copy, and the next 401 clears
// it here.
...
export function clearCredential(): void {
  ...
  try {
    ...
    // (e.g. right after `dimi web rotate-token`).
    const raw = globalThis.localStorage?.getItem(STORAGE_KEY);
    const stored = raw === null || raw === undefined
      ? undefined
      : decodeStoredCredential(raw);

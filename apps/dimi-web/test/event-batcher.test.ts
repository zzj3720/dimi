 ...
 * Run: pnpm --filter @dimi-agent/dimi-web exec vitest run test/event-batcher.test.ts
 */
...
describe("useDimiWebClient (resync integration)", () => {
  it("flushes queued deltas around an authoritative snapshot before live streaming resumes", async () => {
    ...
    let handlers: DimiEventHandlers | undefined;
    ...
    let seedCalls = 0;
    const connection: DimiEventConnection = {
      subscribe: vi.fn(),
      unsubscribe: vi.fn(),
    ...
    try {
      const { useDimiWebClient } = await import("../src/composables/useDimiWebClient");

...
import { ErrorCodes, DimiError, type DimiErrorCode } from '#/errors';
...
export class Session {
   ...
   * (subagent model binding). Persist the recipe via `DimiHarness.setConfig`
   ...
   * session) reaches a terminal state. Used by `dimi -p` after the main agent's
  ...
  /**
   * Used by `dimi -p` after the main agent's turn ends with `reason ===
   ...
   * bounded session lifetime (e.g. `dimi -p`) poll this to decide whether
  ...
  private ensureOpen(): void {
    if (this.closed) {
      throw new DimiError(ErrorCodes.SESSION_CLOSED, 'Session is closed');
    }
  }

  private requireSummary(): SessionSummary {
    if (this.summary === undefined) {
      throw new DimiError(ErrorCodes.INTERNAL, 'Session summary is unavailable');
    }
    return this.summary;
  }
}
...
function normalizePromptInput(input: string | PromptInput): PromptInput {
  if (typeof input === 'string') {
    if (input.trim().length === 0) {
      throw new DimiError(ErrorCodes.REQUEST_PROMPT_INPUT_EMPTY, 'Prompt input cannot be empty');
    }
...
function normalizeRequiredString(
  ...
  if (normalized.length === 0) {
    throw new DimiError(code, message);
  }
  return normalized;
}

...
import { createDimiHarness, ImageLimits, DimiHarness, SDKRpcClientBase } from '#/index';
...
/**
 * The recursive RPC surface DimiHarness touches for the tests below: kept
...
describe('DimiHarness imageLimits', () => {
  it('exposes the in-process core [image] limits loaded from config.toml', async () => {
    const homeDir = await mkdtemp(join(tmpdir(), 'dimi-sdk-harness-'));
...
`,
  ...
  it('a hand-built harness returns the injected ImageLimits as-is', () => {
    ...
    const harness = new DimiHarness(new StubRpc(), {
      homeDir: '/tmp/home',

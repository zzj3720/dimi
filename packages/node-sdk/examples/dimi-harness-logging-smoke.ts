...
import { createDimiHarness, log } from '@dimi-agent/dimi-sdk';
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'dimi.log' || file.startsWith('dimi.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('dimi.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('dimi.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  ...
  const harness = createDimiHarness({
    identity: { userAgentProduct: 'dimi-cli', version: '0.1.1' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        '  - logs/dimi.log',
        '  - logs/dimi.log.1',
        `  - ${summary.sessionDir}/logs/dimi.log`,
        `  - ${summary.sessionDir}/logs/dimi.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - dimi.log.1 exists',
        '  - dimi.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/dimi.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/dimi.log',
        '  - global rotated logs such as ~/.dimi-test/logs/dimi.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',

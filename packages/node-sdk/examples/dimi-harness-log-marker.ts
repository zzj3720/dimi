...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/node-sdk/examples/dimi-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     Dimi home dir; defaults to DIMI_CODE_HOME or ~/.dimi
...
async function main(): Promise<void> {
  ...
  const harness = createDimiHarness({
    identity: { userAgentProduct: 'dimi-cli', version: 'log-marker' },
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'dimi.log');
    const payload = {

...
function snapshot(ctx: TestAgentContext, phase: string) {
  ...
  const tasks = ctx.get(IAgentTaskService).list(false).map((t) => ({
    kind: t.kind,
    status: t.status,
...
describe('token compare complex (worktree async)', () => {
  ...
  it(
    'mixed multi-wave: fast inline + slow detach + staggered completions',
    async () => {
      const fastRead = new ControllableTool('FastRead', 'Read a small snippet quickly.');
      const slowSearch = new ControllableTool(
        'SlowSearch',
        'Search a large corpus (may run long).',
      );

...
//
// Run:  node bench/import-dimi.js [--data ~/.dimi] [--out <dir>]
...
const OUT = path.resolve(arg('out', path.join(os.tmpdir(), 'minidb-dimi-' + Date.now())));
...
async function main() {
  ...
  const byWs = db.findEq('byWorkspace', 'dimi-dev-1');
  console.log(`  index lookup (workspace=dimi-dev-1) -> ${byWs.length} in ${(performance.now() - w0).toFixed(2)} ms`);
